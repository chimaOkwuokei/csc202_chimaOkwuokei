...
const App: React.FC = () => {
  ...
  const sqliteDataSource = useSqliteDataSource();  //We need to hold the dataSource once opened, so we can pass it to components for connection. 
  const [dataSource, setDataSource] = useState<DataSource | null>(null)  //declare the function that we will use to get data source from the hook  
  ...
  const display = () => {
    if (dataSource) {
      ...
      return (
        <Text> Cannot find data connection </Text>
      )
    }
  }
  return (

...
const Spreadsheet: React.FC<Props> = ({ entries }) => {
    ...
    const makeTable = () => {
        ...
        const table = {
            ...
            headers: entriesToShare.length > 0? ["", "S/N", "clinicDate", "natureOfAilment", "medicinePrescribed", "Procedure Undertaken","Date of next appointment"] : [],//first empty space is for id in row which is not shown but kept for reference
            rows: entriesToShare.map((entry) => {
                return [entry.id, moment([entry.txnDay!, entry.txnMonth!, entry.txnDay!]).format("LL"), entry.clinicDate, entry.natureOfAilment, entry.medicinePrescribed,entry.procedureUndertaken,entry.dateOfNextAppointment]
            }),
            widthArr: [0, 50, 70, 120, 100, 80,100, 80]//first space is for the sake of id which is not shown
        };
        setTable(table);

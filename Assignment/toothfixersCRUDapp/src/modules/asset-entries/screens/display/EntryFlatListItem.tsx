...
const EntryFlatListItem: React.FC<Props> = ({ item }) => {
    ...
    return (
        <View style={styles.inputContainerStyle}>
            ...
            <Text style={{ fontSize: 18 }}>First Name: {item.clinicDate}</Text>
        <Text style={{ fontSize: 18 }}>Surname: {item.natureOfAilment}</Text>
        <Text style={{ fontSize: 18 }}>Middle Name: {item.medicinePrescribed}</Text>
        <Text style={{ fontSize: 18 }}>Date of Birth: {item.procedureUndertaken}</Text>
        <Text style={{ fontSize: 18 }}>Home Address: {item.dateOfNextAppointment}</Text>
        <ButtonGroup

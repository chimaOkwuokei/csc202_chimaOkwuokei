...
const Spreadsheet: React.FC<Props> = ({ entries }) => {
    ...
    const makeTable = () => {
        ...
        const table = {
            ...
            headers: entriesToShare.length > 0? ["", "S/N", "FirstName", "Surname", "MiddleName", "Date of birth","HomeAddress", "Date Of Registration", "Matriculation Number"] : [],//first empty space is for id in row which is not shown but kept for reference
            rows: entriesToShare.map((entry) => {
                return [entry.id, moment([entry.txnYear!, entry.txnMonth!, entry.txnDay!]).format("LL"), entry.firstName, entry.surName, entry.middleName,entry.dateOfBirth,entry.homeAddress,entry.dateOfRegistration,entry.matriculationNumber]
            }),

            widthArr: [0, 40, 70, 120, 100, 80, 70, 120, 100, 80]//first space is for the sake of id which is not shown
        };
        setTable(table);
...
const styles = StyleSheet.create({
    ...
    head: { height: 40, backgroundColor: 'pink', width: '100%' },
    row: { flexDirection: 'row', backgroundColor: 'pink', borderWidth: 1, borderColor: 'pink' },
    text: { padding: 6 },
